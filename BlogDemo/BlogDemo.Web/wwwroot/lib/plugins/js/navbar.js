	...
	var navbar = {
		...
		bind: function (callback, params) {
			...
			_target.find('a[kit-target]').each(function () {
				...
				_target.find('a[displayzx]').each(function () {
				var _that = $(this),
					tipsId = undefined;
				if (defaults.showTips) {
					_that.hover(function () {
			...
			////样式
			//var buttons = _doc.find('li.layui-nav-item');
			//buttons.find('dd').on('click', function () {
			//	$(this).find('a').addClass('layui-bg-blue');
			//	$(this).siblings().find('a').removeClass('layui-bg-blue');
			//});
			//样式
			var buttons = _doc.find('dl.layui-nav-child dd a');
			var buttonsCount = buttons.length;
			for (var i = 0; i <= buttonsCount; i += 1) {
				buttons[i].onclick = function (e) {
					buttons.removeClass('layui-bg-blue');
